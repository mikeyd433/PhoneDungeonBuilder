...
| One key, two doors | `unique (from_node_id, digit)` was dropped in 0019: a room with readings has a wall per state, so "press 2" may be a different door in each — its own label and its own destination. The real rule, **no two doors on one key in the SAME state**, depends on `hidden_doors` and on which readings exist, so it is checked in `features/room/keys.ts` and reported (room marker, editor, export warning) rather than refused. The export emits **one** transition per digit into `<slug>_d<n>_pick`, a split on the reading number — two `Digits equals 2` transitions off one split would have Studio silently take the first. Widgets for the second door on a key are named `_d2b_`, because duplicate names are error 81022's second cause. A blank arch offers a key free **in the state being stood in**, which is how the second door gets made; chiselling there hides it in every other state. |
| Standing in one state | The room view takes a state — `'all'` (default, the authoring view: every door, marked), `null` (as written), or a reading id — and renders as that caller finds it: that reading's words **as the room's script** (not a banner over it), only the doors it offers, and `withheldExits` listing the rest so a door you took away can be given back. Reached from a plate in the room's top-left corner, and switching re-keys the room so it slides in the way walking into one does — because to the caller it belongs to, this IS a different room. The editor follows: `EditorSheet` takes `viewing`, so the Narration box edits that reading's words and the exits list is that state's wall. Title, design and notes stay the room's. Chips read `Has lamp` / `No lamp` (`shortCondition`, article stripped), and the plate only exists on a room with readings. |
