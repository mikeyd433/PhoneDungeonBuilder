...
| One key, two ROOMS | The plainest answer to "press 1 goes somewhere else if you have the helmet", and usually the right one: a **fork on the door** (`ForkSheet`), reached beside the destination it splits. Underneath it is a `divert` gate — in the schema since 0001, exported and playtested the whole time, but spelled "Require something → if they can't → send them somewhere else" three levels into the Items tab. Two ordinary rooms, each with its own name, script, cast and exits, so nothing is shared and nothing is a state. **A divert is a fork, not a refusal**, so the door's reaction now wraps the whole split rather than the passing branch — both routes hear it, in the export and in the playtest. Choice effects still fire on the pass route only; the UI says so. **Walking through a forking door stops at `ForkGate`** and asks which of the two rooms to stand in — the same failing `ReactionGate` fixes, that what lives BETWEEN two rooms can be seen from neither, and without it the fail route was a room you had built and could only reach by the map. A route with nowhere to go yet is wired from that modal. |
| A divert is an EDGE | `graphEdges` emits `kind: 'divert'` for a diverting gate's `fail_node_id`. It never did, so the map never drew the far side of a fork and the ledger reported that room as sealed — the identical failure that made the fight edges necessary. Excluded from the solver's choices (like reading edges): it reaches that room through the gate on the choice, which is where the condition lives. |
| One key, two doors | `unique (from_node_id, digit)` was dropped in 0019: a room with readings has a wall per state, so "press 2" may be a different door in each — its own label and its own destination. The real rule, **no two doors on one key in the SAME state**, depends on `hidden_doors` and on which readings exist, so it is checked in `features/room/keys.ts` and reported (room marker, editor, export warning) rather than refused. The export emits **one** transition per digit into `<slug>_d<n>_pick`, a split on the reading number — two `Digits equals 2` transitions off one split would have Studio silently take the first. Widgets for the second door on a key are named `_d2b_`, because duplicate names are error 81022's second cause. The room view no longer builds them: its blank arch takes the first key **no door uses at all**, because the author's wall is one wall and a new door there belongs to no state. The doors that exist on a shared key are drawn together and marked. Ticking which reading offers which is the Readings tab's grid. |
| Standing in one state | **Cut.** There was a plate in the room's top-left corner that stood you in the room as one kind of caller — that reading's words, only the doors it offers, a "not offered here" list for the rest. It went: the job it was built for ("press 1 goes somewhere else if you have the helmet") is a **fork on the door**, which needs no per-state view because it is two ordinary rooms with a check between them. What is left is the author's view and only the author's view: every door on the wall, with the conditional ones **marked** rather than removed — which is what the switcher was genuinely worse at, since a door you cannot see is a door you have to remember exists. Readings are still written and their doors still ticked, in the editor's Readings tab, where the words are. |
